...
# =============================================================================
# BOVI 보드게임 채팅봇 완전 자동 EC2 배포 스크립트 (Static Files 문제 해결)
...
log_info "🚀 BOVI 보드게임 채팅봇 완전 자동 배포 시작 (Static Files 문제 해결)..."
...
# 7. static 및 media 디렉토리 설정 (중요!)
log_info "📁 Static Files 디렉토리 설정..."
mkdir -p "$PROJECT_DIR/static"
mkdir -p "$PROJECT_DIR/staticfiles"
mkdir -p "$PROJECT_DIR/media"

# static 파일들이 있는지 확인하고 복사
if [ -f "$PROJECT_DIR/static/chatbot/logo.png" ]; then
    log_info "✅ logo.png 파일 확인됨"
else
    log_warning "⚠️ logo.png 파일이 없습니다. 기본 파일을 생성합니다."
    mkdir -p "$PROJECT_DIR/static/chatbot"
    # 빈 이미지 파일 생성 (실제로는 로고 파일을 직접 복사해야 함)
    touch "$PROJECT_DIR/static/chatbot/logo.png"
fi

# 8. Django 설정 및 Static Files 수집
log_info "🔧 Django 프로젝트 설정 및 Static Files 수집..."
python manage.py makemigrations
python manage.py migrate
python manage.py collectstatic --noinput --clear

# static 파일이 제대로 수집되었는지 확인
if [ -f "$PROJECT_DIR/staticfiles/chatbot/logo.png" ]; then
    log_success "✅ Static Files 수집 완료: logo.png 확인됨"
else
    log_warning "⚠️ logo.png가 수집되지 않았습니다. 수동으로 복사합니다."
    mkdir -p "$PROJECT_DIR/staticfiles/chatbot"
    cp "$PROJECT_DIR/static/chatbot/logo.png" "$PROJECT_DIR/staticfiles/chatbot/" 2>/dev/null || true
fi
...
log_success "Django 설정 및 Static Files 수집 완료"
